...
description: "통합 로그인 설계에서 IdP, authorization code, PKCE, BFF, sender-constrained token이 각각 어떤 문제를 해결하는지 정리한 글입니다."
pubDate: 2026-06-30
updatedDate: 2026-06-30
category: "Security"
tags: ["SSO", "OIDC", "OAuth2", "PKCE", "BFF", "Authentication"]
techStacks: ["Java / Spring", "Security / Auth"]
featured: true
priority: 1
proof: "통합 로그인 설계 정리"
---
...
`PKCE`는 `Proof Key for Code Exchange`의 약자입니다. Authorization Code Flow에서 code를 token으로 교환할 때, “이 code를 교환하러 온 주체가 처음 로그인 흐름을 시작한 바로 그 client인가”를 확인하는 장치입니다.

흐름은 이렇습니다.

```text
1. client가 랜덤 문자열 code_verifier를 만든다.
2. code_verifier를 해시해서 code_challenge를 만든다.
3. 로그인 요청에는 code_challenge만 보낸다.
4. IdP는 authorization code와 code_challenge의 관계를 기억한다.
5. token 교환 시 client가 code_verifier를 제출한다.
6. IdP가 code_verifier를 해시해 기존 code_challenge와 비교한다.
```
...
공격자가 redirect 과정에서 authorization code만 훔쳤다고 해도, `code_verifier`를 모르면 token으로 교환할 수 없습니다.
...
PKCE는 원래 모바일 앱이나 SPA처럼 `client_secret`을 안전하게 숨길 수 없는 public client를 위해 중요했습니다. 하지만 지금은 confidential client에서도 defense-in-depth로 쓰는 경우가 많습니다. BFF도 서버에 `client_secret`을 둘 수 있지만, PKCE를 같이 적용하면 authorization code 탈취나 redirect 흐름 변조에 대한 방어가 한 겹 더 생깁니다.

정리하면 PKCE는 access token을 보호하는 장치라기보다, **authorization code를 훔쳐 token으로 교환하는 공격을 막는 장치**입니다.
...
```text
accessToken:
  - 로그인/refresh 응답 body로 내려준다.
  - SPA가 메모리에 들고 있다.
  - API 호출 때 Authorization header에 넣는다.

refreshToken:
  - HttpOnly / Secure / SameSite cookie로 내려준다.
  - JS에서는 읽을 수 없다.
  - refresh endpoint에서만 사용한다.

server session:
  - Redis 같은 서버 저장소에 sessionId, userId, refreshToken을 저장한다.
  - accessToken 안의 sid와 매 요청마다 비교한다.
```
...
```text
하이브리드 SPA:
  브라우저 JS가 accessToken을 안다.
  XSS가 있으면 accessToken 문자열이 빠져나갈 수 있다.

BFF + SPA:
  브라우저 JS가 accessToken을 모른다.
  accessToken은 BFF 서버 쪽에만 있다.
  브라우저는 HttpOnly session cookie만 보낸다.
```
...
- 쿠키는 브라우저와 도메인 중심 메커니즘이다.
- 내부 서비스 간 호출은 browser cookie보다 Bearer token이 명확하다.
- `aud`, `scope`, `role`, `tenant`, `sub` 같은 claim으로 권한 경계를 표현하기 좋다.
- API Gateway에서 token 검증과 라우팅을 처리하기 쉽다.
- 모바일, 웹, 외부 파트너 API까지 같은 인증 모델을 공유하기 좋다.

그래서 고도화된 구조는 access token을 없애는 것이 아닙니다. **access token을 브라우저가 직접 들지 않게 하고, BFF/API Gateway/내부 서비스 경계에서 다루게 하는 것**에 가깝습니다.
...
`sender-constrained token`은 이 문제를 줄이기 위한 방식입니다. token을 특정 sender에게 묶어서, token 문자열만 훔쳐서는 사용할 수 없게 만듭니다.
...
```text
token:
  이 access token은 certificate A를 가진 client만 사용할 수 있다.
...
DPoP는 `Demonstrating Proof of Possession`의 약자입니다. client가 개인키를 가지고 있고, 매 요청마다 “내가 이 token에 묶인 키를 소유하고 있다”는 증명 JWT를 같이 보냅니다.
...
그래서 “가장 고도화된 방식이 BFF + SPA인가?”라고 묻는다면, 웹 브라우저 프론트 기준으로는 그렇다고 볼 수 있습니다. 더 정확히는 **BFF + SPA를 기본 경계로 두고, 내부 MSA에는 token strategy와 sender-constrained token을 선택적으로 얹는 구조**가 더 완성형에 가깝습니다.
...
- refresh token을 HttpOnly cookie에 숨기고 server session과 묶는 하이브리드 구조는 순수 stateless JWT보다 통제력이 좋다.
- BFF는 access token과 refresh token을 서버에 숨기고, 브라우저에는 HttpOnly session cookie만 주는 구조다.
- MSA에서는 access token을 없애기보다, 브라우저가 아니라 BFF/API Gateway/내부 서비스 경계에서 다루게 하는 방향이 자연스럽다.
- PKCE는 authorization code를 훔쳐 token으로 교환하는 공격을 막는다.
- sender-constrained token은 token 문자열만 훔쳐서는 재사용할 수 없게 token을 특정 sender에 묶는다.
